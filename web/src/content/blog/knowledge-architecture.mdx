...
Everything else — the predictive scorer, the hybrid search, the reranker — is enhancement. They make a good system better. But the foundation is the structure itself. For technical implementation details, see the [[docs/knowledge-architecture|knowledge architecture docs]].
...
We call this concept [[docs/desire-paths|desire paths]] — like the worn trails in a park that reveal where people actually walk, the knowledge graph learns which traversals are productive and reinforces them over time. The fundamental shift this architecture makes is from *search* to *traversal*.
...
Signet's predictive scorer runs on top of this — a neural network trained on your interaction patterns that already holds the highest relevance accuracy of any retrieval method in the system. It learns which aspects matter most for which kinds of sessions, which attributes to prioritize, which dependencies are worth following. It anticipates what context will be needed before the agent asks, before a prompt is submitted. It operates on a candidate pool that's already structurally coherent — not a flat bag of facts.
